...
def compute_v1_curvature(features):
    ...
    for fn in range(1, len - 1):
        ...
        cos_alpha = np.arccos(numerator / denominator)*(np.linalg.norm(next - prev))
        #cos_alpha = 2*np.sin(np.arccos(numerator / denominator))/np.linalg.norm(next - prev)
        # if np.linalg.norm(prev - cur)>np.linalg.norm(next - cur):
        #     cos_alpha = (np.arccos(numerator / denominator))*((np.linalg.norm(next - cur))*np.abs(np.linalg.norm(prev - cur)-np.linalg.norm(next - cur)))#/(np.dot((next - cur).T, (next - cur)).squeeze() + np.dot((prev - cur).T, (prev - cur)).squeeze()+ 2*np.linalg.norm(prev - cur)*np.linalg.norm(cur - next)*(numerator / denominator))
        # else:
        #     cos_alpha = (np.arccos(numerator / denominator))*((np.linalg.norm(prev - cur))*np.abs(np.linalg.norm(prev - cur)-np.linalg.norm(next - cur)))#/(np.dot((next - cur).T, (next - cur)).squeeze() + np.dot((prev - cur).T, (prev - cur)).squeeze()+ 2*np.linalg.norm(prev - cur)*np.linalg.norm(cur - next)*(numerator / denominator))
...
def compute_lgn_curvature(features):
    ...
    for fn in range(1, len - 1):
        ...
        numerator = np.dot((next - cur).T, (cur-prev)).squeeze()
        denominator = np.linalg.norm(next - cur) * np.linalg.norm(cur - prev)
        cos_alpha = np.log(1+np.arccos(numerator / denominator)*np.sqrt(np.linalg.norm(next - prev)))
        #cos_alpha = 2*np.sin(np.arccos(numerator / denominator))/np.linalg.norm(next - prev)
        # if np.linalg.norm(prev - cur)>np.linalg.norm(next - cur):
        #     cos_alpha = (np.arccos(numerator / denominator))*np.sqrt((np.linalg.norm(next - cur))*np.abs(np.linalg.norm(prev - cur)-np.linalg.norm(next - cur)))#/(np.dot((next - cur).T, (next - cur)).squeeze() + np.dot((prev - cur).T, (prev - cur)).squeeze()+ 2*np.linalg.norm(prev - cur)*np.linalg.norm(cur - next)*(numerator / denominator))
        # else:
        #      cos_alpha = (np.arccos(numerator / denominator))*np.sqrt((np.linalg.norm(prev - cur))*np.abs(np.linalg.norm(prev - cur)-np.linalg.norm(next - cur)))#/(np.dot((next - cur).T, (next - cur)).squeeze() + np.dot((prev - cur).T, (prev - cur)).squeeze()+ 2*np.linalg.norm(prev - cur)*np.linalg.norm(cur - next)*(numerator / denominator))
        curvatures[fn - 1] = cos_alpha
...
def clear_data(data):
    ...
    # data = data[:, ~np.isposinf(data).any(axis=0)]
    # data = data[:, ~np.isneginf(data).any(axis=0)]
